"""Tests for the offline selector engine (Phase 11b).
...
something it never tested, which is the exact failure AUDIT G3 records.
"""
...
<html><body>
  <div id="root" class="wrap">
    <div data-testid="mainFeed">
      ...
      <a data-view-name="feed-actor-image" href="/in/aaa/"><img></a>
      <div><a href="/in/aaa/">Aaa</a></div>
    </div>
...
def test_xpath_matches_on_visible_text_not_aria_label(count):
    """The 2026-07-31 trap, asserted directly.
    ...
    reads "Comment". Only the second is the submit button.
    """
    assert count("//button[normalize-space(.)='Comment']", xpath=True) == 1
...
def test_xpath_ignores_script_text(count):
    ...
    assert count("//script[normalize-space(.)='Comment']", xpath=True) == 0


def test_xpath_attribute_and_bare_forms(count):
    assert count("//div[@role='listitem']", xpath=True) == 3
    assert count("//button", xpath=True) == 4
...
def test_every_registry_selector_is_supported():
    ...
    for key, spec in shc.SELECTOR_REGISTRY.items():
        for selector in spec["selectors"]:
            try:
                count(selector, xpath=bool(spec.get("xpath")))
