...
def test_new_bin_equals_review_posts_tab(client, env):
    ...
    counts = client.get("/api/posts/jeff/lifecycle").get_json()["counts"]
    tab = client.get("/api/posts/jeff").get_json()
    assert counts["NEW"] == 5
...
def test_review_posts_enriched_from_scrape_file(client, env):
    ...
    post = client.get("/api/posts/jeff").get_json()["posts"][0]
...
def test_generated_bin_equals_generated_tab_with_drafts(client, env):
    ...
    counts = client.get("/api/posts/jeff/lifecycle").get_json()["counts"]
    tab = client.get("/api/posts/jeff/lifecycle?status=GENERATED").get_json()
...
def test_generated_tab_shows_drafts_even_when_comment_files_archived(client, env):
    """The GENERATED-tab bug: drafts got archived so the file-reading view showed
    none. The store-driven Generated tab still shows all of them with drafts,
    while the Review-Comments sub-view (get_comments) correctly stays empty."""
    ...
    generated_tab = client.get("/api/posts/jeff/lifecycle?status=GENERATED").get_json()
    review_queue = client.get("/api/comments/jeff").get_json()

    assert len(generated_tab["posts"]) == 3                 # Generated tab: all shown
    assert review_queue["comments"] == []                   # Review Comments: archived excluded
...
def test_reconcile_recovers_generated_draft_from_active_file(env):
    ...
    stats = {}
    post_store.reconcile("jeff", stats=stats)
    rec = PostStore("jeff").get(ACTIVITY.format(1))
...
def test_reconcile_generated_recovery_is_idempotent(env):
    ...
    first = post_store.reconcile("jeff")
    second = post_store.reconcile("jeff")
...
def test_bin_count_matches_tab_after_mixed_lifecycle(client, env):
    ...
    counts = client.get("/api/posts/jeff/lifecycle").get_json()["counts"]
    new_tab = client.get("/api/posts/jeff").get_json()["posts"]
    gen_tab = client.get("/api/posts/jeff/lifecycle?status=GENERATED").get_json()["posts"]
