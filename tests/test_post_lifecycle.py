...
def test_migration_classifies_from_legacy_files(env):
    ...
    counts = post_store.migrate_from_legacy("jeff")
...
def test_migration_is_idempotent(env):
    ...
    _write(env.comments / "posting_progress.json",
           {"posted_comments": [ACTIVITY.format(1)]})
    first = post_store.migrate_from_legacy("jeff")
    second = post_store.migrate_from_legacy("jeff")
...
def test_lifecycle_endpoint_returns_counts_and_groups(client, env):
    ...
    data = client.get("/api/posts/jeff/lifecycle").get_json()
...
def test_lifecycle_status_filter_returns_trash_with_reason(client, env):
    ...
    data = client.get("/api/posts/jeff/lifecycle?status=TRASH").get_json()
...
def test_reject_then_excluded_from_review_and_in_trash(client, env):
    ...
    client.get("/api/posts/jeff/lifecycle")

    # Reject post 1.
    resp = client.post("/api/posts/jeff/reject", json={"key": ACTIVITY.format(1)})
    assert resp.status_code == 200
    assert resp.get_json()["counts"]["TRASH"] == 1

    # It disappears from the Review-Posts list...
    review = client.get("/api/posts/jeff").get_json()
    ...
    trash = client.get("/api/posts/jeff/lifecycle?status=TRASH").get_json()
...
def test_restore_brings_post_back(client, env):
    _write(env.timeline / "ai_posts_1.json", {"quality_posts": [_post(1)]})
    client.get("/api/posts/jeff/lifecycle")
    client.post("/api/posts/jeff/reject", json={"key": ACTIVITY.format(1)})

    resp = client.post("/api/posts/jeff/restore", json={"key": ACTIVITY.format(1)})
    assert resp.status_code == 200
...
def test_reject_unknown_is_404(client, env):
    _write(env.timeline / "ai_posts_1.json", {"quality_posts": [_post(1)]})
    client.get("/api/posts/jeff/lifecycle")
    resp = client.post("/api/posts/jeff/reject", json={"key": "https://nope/x"})
    assert resp.status_code == 404


def test_reject_requires_identifier(client, env):
    resp = client.post("/api/posts/jeff/reject", json={})
    assert resp.status_code == 400


def test_get_posts_annotates_status_and_key(client, env):
    _write(env.timeline / "ai_posts_1.json", {"quality_posts": [_post(1)]})
    data = client.get("/api/posts/jeff").get_json()
