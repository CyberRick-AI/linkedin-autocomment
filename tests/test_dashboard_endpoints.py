...
def test_get_comments_empty(api_client):
    resp = api_client.get("/api/comments/jeff")
    assert resp.status_code == 200
...
def test_save_endpoint_reports_count(api_client, comments_dir, make_comment):
    comments = [make_comment(i) for i in range(1, 4)]
    resp = api_client.post("/api/comments/jeff/save", json={"comments": comments})
    assert resp.status_code == 200
...
def test_get_comments_returns_both_field_conventions(api_client, comments_dir, make_comment):
    ...
    out = api_client.get("/api/comments/jeff").get_json()["comments"]
    assert len(out) == 2
    for c in out:
...
def test_reset_config(api_client):
    api_client.post("/api/profiles/jeff/config", json={"display_name": "Custom"})
    resp = api_client.post("/api/profiles/jeff/config/reset")
    assert resp.status_code == 200
