...
def test_writes_png_sidecar_and_html(failures_dir):
    path = fc.capture_failure(FakeDriver(), "feed_no_posts", profile_name="jeff")
...
def test_returns_the_saved_path(failures_dir):
    path = fc.capture_failure(FakeDriver(), "x", profile_name="jeff")
...
def test_page_source_can_be_disabled(failures_dir):
    path = fc.capture_failure(FakeDriver(), "x", profile_name="jeff", page_source=False)
    assert not os.path.exists(path[:-4] + ".html")


def test_label_is_sanitized_for_filename(failures_dir):
    path = fc.capture_failure(FakeDriver(), "weird label/with:chars", profile_name="jeff")
...
def test_screenshot_raising_returns_none_and_does_not_throw(failures_dir):
    path = fc.capture_failure(FakeDriver(raise_screenshot=True), "boom", profile_name="jeff")
...
def test_screenshot_false_returns_none(failures_dir):
    assert fc.capture_failure(FakeDriver(screenshot_ok=False), "x", profile_name="jeff") is None
...
def test_context_reads_are_guarded(failures_dir):
    ...
    path = fc.capture_failure(DeadDriver(), "x", profile_name="jeff")   # must not throw
...
def test_poster_captures_each_step(monkeypatch):
    ...
    def run(open_ok, type_ok, submit_ok):
        calls = []
