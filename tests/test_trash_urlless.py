...
def test_reconcile_trashes_urlless_new(env):
    ...
    assert PostStore("jeff").get("hash:2")["trash_reason"] == REASON_NO_URL
...
def test_reconcile_does_not_trash_commented_urlless(env):
    ...
    assert PostStore("jeff").get("hash:1")["status"] == COMMENTED
...
def test_restore_then_reconcile_retrashes_urlless(env):
    ...
    post_store.reconcile("jeff")
    assert PostStore("jeff").get(key)["trash_reason"] == REASON_NO_URL
...
def test_scraper_upserts_urlless_engageable_as_no_url(env):
    ...
    fake = SimpleNamespace(
        profile_name="jeff", posts=[good, urlless, low], trashed=[],
