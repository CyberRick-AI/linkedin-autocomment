...
def test_reconcile_downgrades_falsely_new_to_commented(env):
    ...
    counts = post_store.reconcile("jeff")
    assert counts == {"NEW": 1, "GENERATED": 0, "COMMENTED": 1, "TRASH": 0}
    assert PostStore("jeff").get(ACTIVITY.format(1))["status"] == COMMENTED
...
def test_reconcile_downgrades_falsely_new_to_generated(env):
    ...
    gen = PostStore("jeff").get(ACTIVITY.format(2))
...
def test_reconcile_reads_ready_files_too(env):
    ...
    assert PostStore("jeff").get(ACTIVITY.format(5))["status"] == GENERATED
...
def test_reconcile_does_not_resurrect_manual_trash(env):
    ...
    post_store.reconcile("jeff")
    assert PostStore("jeff").get(ACTIVITY.format(9))["status"] == TRASH
...
def test_reconcile_does_not_downgrade_commented(env):
    ...
    post_store.reconcile("jeff")
    assert PostStore("jeff").get(ACTIVITY.format(4))["status"] == COMMENTED
...
def test_load_synced_store_reconciles_generated(env):
    ...
    store = post_store.load_synced_store("jeff")
...
def test_generate_uses_store_new_not_latest_file(env, client, captured_jobs):
    ...
    resp = client.post("/api/comments/jeff/generate", json={"limit": None})
...
def test_generate_new_count_matches_store_bin(env, client, captured_jobs):
    ...
    resp = client.post("/api/comments/jeff/generate", json={})
    assert resp.status_code == 200
    infile = captured_jobs[0]["args"][1]
    data = json.loads(open(infile, encoding="utf-8").read())
    store = PostStore("jeff")
...
def test_generate_explicit_input_file_overrides_store(env, client, captured_jobs):
    """An explicit input_file still wins (legacy/advanced path)."""
    _seed_store([_rec(1, NEW)])
    explicit = env.timeline / "ai_posts_curated.json"
    _write(explicit, {"quality_posts": [_post(50)]})
    resp = client.post("/api/comments/jeff/generate", json={"input_file": str(explicit)})
    assert resp.status_code == 200
    assert captured_jobs[0]["args"][1] == str(explicit)
