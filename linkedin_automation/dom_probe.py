...
The selector watchdog and ``tools/selector_probe.py`` both need to answer one
question: how many elements does this selector match? Against a live page
Selenium answers it inside the browser. Against a **saved** page there is no
browser, and that is the case this module exists for: it is what makes the
selector gate runnable in CI, on a fixture, with no LinkedIn session.
...
all-clear this project already lived through once (see AUDIT G3). So every
...
matters: ``SUBMIT_BUTTON_XPATH`` finds the comment submit button by visible
text. Counting it with CSS is impossible, and skipping it would leave the
highest-consequence selector in the project unchecked.
"""
...
def make_counter(html: str):
    """Return ``count(selector, xpath=False) -> int`` bound to parsed ``html``.

    This is the offline stand-in for the live ``len(driver.find_elements(...))``
    counter, so the same ``check_registry`` logic runs against a fixture and
    against a browser without knowing which it has.
    """
    root = parse_html(html)

    def count(selector: str, xpath: bool = False) -> int:
        nodes = select_xpath(root, selector) if xpath else select_css(root, selector)
        return len(nodes)
